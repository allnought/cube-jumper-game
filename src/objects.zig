...
pub const player = struct {
    ...
    fn move(self: *player, obstacles: *map, state: *u8) void {
        ...
        if (!doesCollide(lastCol.type)) {
            ...
            if (self.velocity.x > 0) {self.rect.x = lastCol.rect.x - self.rect.width;}
            else {self.rect.x = lastCol.rect.x + lastCol.rect.width;}
            self.velocity.x = 0;
        }
        ...
        if (!doesCollide(lastCol.type)) {
            ...
            else {self.rect.y = lastCol.rect.y + lastCol.rect.height;}
            self.velocity.y = 0;
        }
    }
